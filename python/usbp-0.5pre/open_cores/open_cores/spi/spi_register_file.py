...
def spi_RegisterFile(
    ...
    def rtl_read_reg():
        if adr_i == (0x60 + C_WB_BASE_ADDR):
            _wb_do.next = rwRegisters[0]

        elif adr_i == (0x68 + C_WB_BASE_ADDR):
            _wb_do.next = rwRegisters[1]

        elif adr_i == (0x70 + C_WB_BASE_ADDR):
            _wb_do.next = rwRegisters[2]

        elif adr_i == (0x7c + C_WB_BASE_ADDR):
            _wb_do.next = rwRegisters[3]

        elif adr_i == (0x64 + C_WB_BASE_ADDR):
            _wb_do.next = roRegisters[0]

        elif adr_i == (0x6c + C_WB_BASE_ADDR):
            _wb_do.next = roRegisters[1]

        elif adr_i == (0x74 + C_WB_BASE_ADDR):
            _wb_do.next = roRegisters[2]

        elif adr_i == (0x78 + C_WB_BASE_ADDR):
    ...
    @always(clk_i.posedge)
    def rtl_rw_stobes(): 
        if adr_i == (0x60 + C_WB_BASE_ADDR) and _wb_ack: 
            if _wb_wr: 
                rwWr[0].next = True 
                rwRd[0].next = False 
            else: 
            ...
            rwWr[0].next = False 
            rwRd[0].next = False 
        if adr_i == (0x68 + C_WB_BASE_ADDR) and _wb_ack: 
            if _wb_wr: 
                rwWr[1].next = True 
                rwRd[1].next = False 
            else: 
            ...
            rwWr[1].next = False 
            rwRd[1].next = False 
        if adr_i == (0x70 + C_WB_BASE_ADDR) and _wb_ack: 
            if _wb_wr: 
                rwWr[2].next = True 
                rwRd[2].next = False 
            else: 
            ...
            rwWr[2].next = False 
            rwRd[2].next = False 
        if adr_i == (0x7c + C_WB_BASE_ADDR) and _wb_ack: 
            if _wb_wr: 
                rwWr[3].next = True 
                rwRd[3].next = False 
            else: 
    ...
    @always(clk_i.posedge)
    def rtl_ro_stobes(): 
        if adr_i == (0x64 + C_WB_BASE_ADDR) and _wb_ack: 
            if _wb_wr: 
                roRd[0].next = False 
            else: 
                roRd[0].next = True 
            ...
            roRd[0].next = False 
        if adr_i == (0x6c + C_WB_BASE_ADDR) and _wb_ack: 
            if _wb_wr: 
                roRd[1].next = False 
            else: 
                roRd[1].next = True 
            ...
            roRd[1].next = False 
        if adr_i == (0x74 + C_WB_BASE_ADDR) and _wb_ack: 
            if _wb_wr: 
                roRd[2].next = False 
            else: 
                roRd[2].next = True 
            ...
            roRd[2].next = False 
        if adr_i == (0x78 + C_WB_BASE_ADDR) and _wb_ack: 
            if _wb_wr: 
                roRd[3].next = False 
            else: 
                roRd[3].next = True 
