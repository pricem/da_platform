...
#    TP_HDR         #
    ...
    @always_comb
    def rtl_fifo_sigs():
        fifo_wr.next  = fifo_wr_spi | fifo_wr_twi | fifo_wr_rmp
        fifo_rd.next  = fifo_rd_spi | fifo_rd_twi | fifo_rd_rmp
        fifo_di.next  = fifo_di_spi | fifo_di_twi | fifo_di_rmp
        
        #if not fifo_full and not fifo_empty:
        #    fifo_wr.next = fifo_do_vld
        #    fifo_rd.next = True
        #else:
        #    fifo_wr.next = False
        #    fifo_rd.next = False
    ...
    @always_comb
    def rtl_wb_bus_or():
        wb_dat_i.next = g0_dat_i | g1_dat_i | spi_dat_i | twi_dat_i | rmp_dat_i
        wb_ack.next   = g0_ack   | g1_ack   | spi_ack   | twi_ack   | rmp_ack
...
def convert(to = 'ver'):
    ...
    if to == 'ver':
        toVerilog(usbp_myhdl, reset, sys_clk,
                  IFCLK, FLAGA, FLAGB, FLAGC, FLAGD,
                  SLOE, SLRD, SLWR,
                  FIFOADR, PKTEND, FDI, FDO, LEDs,
                  SS, SCK, MOSI, MISO,
                  SCL_i, SCL_o, SDA_i, SDA_o) #, TP_HDR)
        
    elif to == 'vhd':
        toVHDL(usbp_myhdl, reset, sys_clk,
               IFCLK, FLAGA, FLAGB, FLAGC, FLAGD,
               SLOE, SLRD, SLWR,
               FIFOADR, PKTEND, FDI, FDO, LEDs,
               SS, SCK, MOSI, MISO,
