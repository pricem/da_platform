...
`include "cellram_parameters.vh"
	...
	/*	Modified by Michael Price 9/13/2012
		Add support for initial data in memory	*/
	...
	initial begin
		if (init_use_file) begin
			//	Load supplied data file for beginning portion
			init_file = $fopen(init_filename, "rb");
			return_code = $fread(memory, init_file);
			$display("Read memory data from %s to registers in %m with return code %d", init_filename, return_code);
		end				
	end
    ...
    task memory_write;
        ...
        begin : memory_write_task
			...
			//	Modified by Michael Price 9/21/2012
			if (SPARSE_MODE) begin
            	...
            	//	Simpler alternative using full array
...
module cellram_pathdelay (
    ...
    //	Modified by Michael Price 10/12/2014
    //	iverilog doesn't support specify blocks, so get as close as we can with continuous assignments
    ...
    */
    ...
    assign #(   tKHTL,  tHZ) clk2waite = clk2waiter;
    assign #(    tALZ, tAHZ) adv2dqe   = adv_n;
    assign adv2dq    = adv2dqr;
    assign adv2wi    = adv2dqr;
    assign #(    tDPD,    0) zz2pd     = !zz_n;
    assign #(     tLZ,  tHZ) ce2dqe    = !ce_n;
    assign #(     tCO,    0) ce2dq     = !ce_n;
    assign #(    tCEM,    0) ce2cem = !ce_n;
    assign #(     tWI,    0) ce2wi     = !ce_n;
    assign #(tCEW_MIN,  tHZ) ce2waite  = !ce_n;
    assign #(tCEW_MAX,    0) ce2wait   = !ce_n;
    assign #(      15,    0) ce2rst    = ce_n;
    assign #(    tOLZ, tOHZ) oe2dqe    = !oe_n;
    assign #(     tOE,    0) oe2dq     = !oe_n;
    assign #(    tBOE,    0) soe2dq    = !oe_n;
    assign #(tOEW_MIN,  tHZ) oe2waite  = !oe_n;
    assign #(tOEW_MAX,    0) oe2wait   = !oe_n;
    assign #(       0, tWHZ) we2dqe    = we_n;
    assign #(       0,    0) we2dq     = we_n;
    assign #(       0,    0) we2waite  = we_n;
    ...
    assign #(    tBLZ, tBHZ) by2dqe    = !(&by_n);
    assign #(     tBA,    0) by2dq     = !(&by_n);
    assign #(     tAA,  tOH) addr2dq   = addr2dqr;
    assign #(     tWI,    0) addr2wi   = addr2wir;
